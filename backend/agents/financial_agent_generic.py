...
class LMStudioToolAgent:
    """Compatibility wrapper that redirects to generic agent with Ollama."""
    
    def __init__(self, base_url: str = "http://localhost:11434"):
        # Convert LM Studio config to Ollama config
    ...
    async def initialize(self):
        ...
        self.agent = GenericFinancialAgent(self.config)
    ...
    async def test_connection(self) -> bool:
        ...
        if not self.agent:
            await self.initialize()
        return await self.agent.test_connection()
...
class FinancialAnalysisAgent:
    """Compatibility wrapper for the new generic agent."""
    
    def __init__(self, mcp_manager=None, config: Optional[LLMConfig] = None):
        self.agent = None
        self.config = config
    
    async def initialize(self):
        """Initialize the agent."""
        self.agent = await get_financial_agent(self.config)
